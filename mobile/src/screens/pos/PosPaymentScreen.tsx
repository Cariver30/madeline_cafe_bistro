...
const PosPaymentScreen = ({navigation, route}: NativeStackScreenProps<PosStackParamList, 'PosPayment'>) => {
  ...
  const {token} = useAuth();
  const {payTicket, addPayment, actionState, error, getTicketById, loadTickets} = usePosTickets();
  ...
  const subtotal = useMemo(() => {
    ...
    const total = ticket.orders.reduce((sum, order) => {
      if (order.status === 'cancelled') {
        return sum;
      }
  ...
  const summarySubtotal = ticket?.payment_summary?.subtotal ?? subtotal;
  const summaryTaxTotal = ticket?.payment_summary?.tax_total ?? 0;
  const summaryTotal =
    ticket?.payment_summary?.total ??
  ...
  const paidTotal = ticket?.payment_summary?.paid_total ?? 0;
  ...
  const splitSubtotal = useMemo(() => {
    if (splitMode === 'items') {
      const selected = new Set(splitItemIds);
      return splitItems
  ...
  const handlePay = async () => {
    ...
    await payTicket(ticketId, selected, tipValue);
    navigation.popToTop();
  };
  ...
  const handleSendReceipt = async () => {
    ...
    try {
      const response = await sendPosReceipt(token, ticketId);
  ...
  return (
    <KeyboardAvoidingView
      ...
      <View style={styles.card}>
        <Text style={styles.title}>Cobrar ticket</Text>
        <Text style={styles.subtitle}>Selecciona metodo de pago</Text>
        ...
        <View style={styles.receiptBox}>
          <View style={styles.receiptHeader}>
            <View>
              <Text style={styles.receiptTitle}>Resumen del ticket</Text>
          ...
          <>
            <View style={styles.methodList}>
              {(['cash', 'card', 'ath'] as const).map(method => (
                <TouchableOpacity
                  key={method}
                  style={[styles.methodButton, splitMethod === method && styles.methodActive]}
                  onPress={() => setSplitMethod(method)}>
                  <Text style={[styles.methodText, splitMethod === method && styles.methodTextActive]}>
            ...
            {splitMethod === 'cash' ? (
              <View style={styles.cashRow}>
                ...
                />
                <View style={styles.cashSummary}>
                  <Text style={styles.cashMeta}>Cambio: {splitChangeDue !== null ? `$${splitChangeDue.toFixed(2)}` : '—'}</Text>
                </View>
              </View>
            ) : null}
        ...
        {paymentMode === 'single' ? (
          <>
            <View style={styles.tipRow}>
              ...
              {tipInvalid ? (
                <Text style={styles.error}>Ingresa un monto valido.</Text>
              ) : null}
            </View>
            {selected === 'cash' ? (
              <View style={styles.cashRow}>
                ...
                <View style={styles.cashSummary}>
                  ...
                  <Text style={styles.cashMeta}>
                    Cambio:{' '}
                    {changeDue !== null ? `$${changeDue.toFixed(2)}` : '—'}
                  </Text>
                </View>
                {cashShort ? (
        ...
        <TouchableOpacity
          ...
          {actionState.payingTicketId === ticketId || terminalBusy ? (
            <ActivityIndicator color="#0f172a" />
          ) : (
