...
<head>
    ...
    <title>Editar categoría · Cantina</title>
    ...
    <div class="max-w-4xl mx-auto py-12 px-4 sm:px-6 lg:px-0 space-y-8">
        ...
        <div class="rounded-3xl border border-white/10 bg-white/5 backdrop-blur p-8 shadow-2xl">
            <div class="space-y-2 mb-8">
                ...
                <p class="text-white/60 text-sm">Actualiza los datos que verá el cliente en la cantina.</p>
            </div>
            ...
            <form action="{{ route('cantina-categories.update', $cantinaCategory->id) }}" method="POST" class="space-y-6">
                @csrf
                @method('PUT')

                <div>
                    <label for="name" class="block text-sm font-semibold text-white/80 mb-2">Nombre interno</label>
                    <input type="text" id="name" name="name" value="{{ old('name', $cantinaCategory->name) }}" required
                ...
                <div>
                    ...
                    <input type="text" id="cover_title" name="cover_title" value="{{ old('cover_title', $cantinaCategory->cover_title) }}"
                           class="block w-full rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-white placeholder-white/40 focus:border-amber-400 focus:ring-amber-400" />
                </div>

                <div>
                    <label for="cover_subtitle" class="block text-sm font-semibold text-white/80 mb-2">Descripción breve</label>
                    <input type="text" id="cover_subtitle" name="cover_subtitle" value="{{ old('cover_subtitle', $cantinaCategory->cover_subtitle) }}"
                           class="block w-full rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-white placeholder-white/40 focus:border-amber-400 focus:ring-amber-400" />
                </div>
                ...
                <label class="flex items-center gap-3 rounded-2xl border border-white/10 bg-white/5 px-4 py-3">
                    ...
                    <input class="h-4 w-4 rounded text-amber-400 focus:ring-amber-400" type="checkbox" id="show_on_cover" name="show_on_cover" value="1" {{ old('show_on_cover', $cantinaCategory->show_on_cover) ? 'checked' : '' }}>
