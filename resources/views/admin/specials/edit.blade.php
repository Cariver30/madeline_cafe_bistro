...
<div class="space-y-6">
    <div class="flex flex-wrap items-center justify-between gap-3">
        <div>
            <h1 class="text-2xl font-semibold text-slate-900">Editar especial</h1>
            <p class="text-sm text-slate-500">Actualiza horarios y items asociados.</p>
        </div>
    ...
    <form action="{{ route('admin.specials.update', $special) }}" method="POST" class="space-y-8">
        @csrf
        @method('PUT')
        @include('admin.specials._form', compact('special', 'scopes', 'selectedCategories', 'selectedItems', 'days'))
        <div class="flex flex-wrap gap-3">
            <button type="submit" class="primary-button w-auto px-6">Guardar cambios</button>
